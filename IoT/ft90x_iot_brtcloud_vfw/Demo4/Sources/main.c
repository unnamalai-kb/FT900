...
//extern WeatherStation_Cfg_t ew_sensor_cfg;
...
static void iot_app_task( void *pvParameters )
{
    ...
    while (1) {
        ...
        tfp_sprintf(topic, "server/%s/req_configuration", iot_utils_getdeviceid());
        memset(payload, 0x0, sizeof(payload));
        plen = 0;
/*        plen += tfp_snprintf(payload + plen,
                sizeof(payload) - plen,"{\"peripherals\":[\"i2c\"]}");*/
        plen += tfp_snprintf(payload + plen,
        sizeof(payload) - plen,"{}");
        ...
        do  {
...
#if ENABLE_UART == 0
#ifdef ENABLE_CNT_PUBLISH
            if(g_tprobe_task_status)
            {
                iot_modem_tprobe_get_sensor_reading(NULL,topic,64,payload,128);
            ...
            if(g_1wire_task_status)
            {
            	int ret = 0;
                uint16_t temp = iot_modem_1wire_get_sensor_reading(NULL,topic,64,payload,128);
                pub_msg(payload,TOPIC_PAYLOAD);
				ret = iot_publish( g_handle, topic, payload, strlen(payload), 1 );
				DEBUG_PRINTF( ">PUB: %s %s\r\n", topic, payload);
            }

			if(g_anemometer_task_status)
			{
                int ret = 0;
                uint32_t adc = iot_modem_adc_get_sensor_reading(NULL,topic,64,payload,128);
                pub_msg(payload,TOPIC_PAYLOAD);
				ret = iot_publish( g_handle, topic, payload, strlen(payload), 1 );
				DEBUG_PRINTF( ">>>PUB: %s%s\r\n", topic, payload);
			}
...
#if (ENABLE_I2C) || (ENABLE_ADC) || (ENABLE_ONEWIRE) || (ENABLE_TPROBE)
...
static void user_subscribe_receive_cb( iot_subscribe_rcv* mqtt_subscribe_recv )
{
    ...
    ///////////////////////////////////////////////////////////////////////////////////
        ...
        if (ucNumber < ADC_COUNT) {
			...
			if (!pProp) {
                ...
                {
                   iot_modem_adc_get_sensor_reading(NULL,topic,64,payload,128);
                    pub_msg(payload,TOPIC_PAYLOAD);
                    int ret = iot_publish( g_handle, topic, payload, strlen(payload), 1 );
                    DEBUG_PRINTF( ">>PUB: %s %s\r\n", topic, payload);
                }
    ...
    //	DPRINTF_INFO("\r\n%s, %s\r\n",topic,payload);
#if ENABLE_ADC
    	prpptr = g_pADCProperties;
    	for(i = 0;i<g_ucADCPropertiesCount;i++){
    		DEVICE_PROPERTIES *prop = (DEVICE_PROPERTIES*)prpptr;
    		if(prop  != NULL){
    			if(prop->m_ucEnabled){
    			 len += tfp_snprintf(payload + len ,sizeof(payload) - len,GET_IND_PAYLOAD,"adc",i+1,DEVICE_CLASS_ANENOMOMETER,1);
    			 payload[len++] = ',';
    			}
    			prpptr+= sizeof(DEVICE_PROPERTIES);
    		}
