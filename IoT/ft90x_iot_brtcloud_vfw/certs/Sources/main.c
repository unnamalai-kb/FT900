...
///////////////////////////////////////////////////////////////////////////////////
/* Task configurations. */
#define IOT_CERTS_TASK_NAME                        "iot_certs"
#define IOT_CERTS_TASK_PRIORITY                    (5)
#if (USE_MQTT_BROKER == MQTT_BROKER_AWS_IOT) || (USE_MQTT_BROKER == MQTT_BROKER_AWS_GREENGRASS)
    #define IOT_CERTS_TASK_STACK_SIZE              (512)
#elif (USE_MQTT_BROKER == MQTT_BROKER_GCP_IOT)
    #define IOT_CERTS_TASK_STACK_SIZE              (1536 + 32)
#elif (USE_MQTT_BROKER == MQTT_BROKER_MAZ_IOT)
    #if (MAZ_AUTH_TYPE == AUTH_TYPE_SASTOKEN)
        #define IOT_CERTS_TASK_STACK_SIZE          (1536 + 16)
    #elif (MAZ_AUTH_TYPE == AUTH_TYPE_X509CERT)
        #define IOT_CERTS_TASK_STACK_SIZE          (1024 + 64)
    #endif
#else
#define IOT_CERTS_TASK_STACK_SIZE                  (768)
#endif
///////////////////////////////////////////////////////////////////////////////////
...
static void iot_credentials_task(void *pvParameters);
...
TaskHandle_t g_iot_certs_handle;
...
void certs_suspend_callback(void)
{
	vPortFree(tls_certificates.ca);
	vPortFree(tls_certificates.cert);
	vPortFree(tls_certificates.pkey);
}

void certs_resume_callback(void)
{
...
int main( void )
{
...
#else
	uart_puts(UART0,"Copyright (C) Bridgetek Pte Ltd\r\n"
#endif
            "-------------------------------------------------------\r\n"
    ...
    kernel_register_suspend_resume_hooks(certs_suspend_callback,certs_resume_callback);

#if !VFW_APP
    if (xTaskCreate( iot_app_task,
#else
    if (kernel_create_task( iot_credentials_task,
#endif
            IOT_CERTS_TASK_NAME,
            IOT_CERTS_TASK_STACK_SIZE,
            NULL,
            IOT_CERTS_TASK_PRIORITY,
            &g_iot_certs_handle ) != pdTRUE ) {
...
static void iot_credentials_task( void *pvParameters )
{
    ...
    /* obtain TLS certificates and MQTT credentials */

    iot_utils_getcertificates( &tls_certificates );

    /*  Initialize MQTT settings/credentials */
    iot_utils_getcredentials( &mqtt_credentials );

    kernel_iot_credentials(&tls_certificates, &mqtt_credentials);
