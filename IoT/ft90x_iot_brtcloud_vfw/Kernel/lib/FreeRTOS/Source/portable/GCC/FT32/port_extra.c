...
int lDebugEnterCriticalSection_Caller;
inline void vPortAssert_vTaskEnterCriticalInISR(void)
{
    if(uPortIsInISR() == portENTER_ISR)
    {
        DPRINTF("ERROR [%d] %s(): Task = %s, Caller = %x\r\n",
                ulPortSysTickGet(),
                __FUNCTION__,
                pxCurrentTCB->pcTaskName,
                lDebugEnterCriticalSection_Caller);
        while(1);
    }
}
