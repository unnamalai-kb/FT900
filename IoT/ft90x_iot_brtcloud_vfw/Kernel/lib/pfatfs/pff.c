...
// Govind: Not used, pff updated to be stack based in case of SDBL/EFBL!!
...
)
{
	...
	do {
		res = disk_readp(dir, dj->sect, (dj->index % 16) * 32, 32, fs)	/* Read an entry */
			? FR_DISK_ERR : FR_OK;
		if (res != FR_OK) break;
...
#if _USE_LCC
#ifdef _EXCVT
	  __flash__ const BYTE cvt[] = _EXCVT;
#endif
#endif
...
)
{
	...
	if (!_FS_32ONLY && !disk_readp(buf, sect, BS_FilSysType, 2, fs) && LD_WORD(buf) == 0x4146)	/* Check FAT12/16 */
		return 0;
	if (_FS_FAT32 && !disk_readp(buf, sect, BS_FilSysType32, 2, fs) && LD_WORD(buf) == 0x4146)	/* Check FAT32 */
		return 0;
	return 1;
}
...
)
{
	...
	if (disk_initialize(fs) & STA_NOINIT)	/* Check if the drive is ready or not */
		return FR_NOT_READY;
#if !LOADER_MIN_SIZE
	util_take_ts();
#endif
	...
	bsect = 0;
	fmt = check_fs(buf, bsect, fs);			/* Check sector 0 as an SFD format */
	if (fmt == 1) {						/* Not an FAT boot record, it may be FDISK format */
		/* Check a partition listed in top of the partition table */
		if (disk_readp(buf, bsect, MBR_Table, 16, fs)) {	/* 1st partition entry */
			fmt = 3;
		} else {
			if (buf[4]) {					/* Is the partition existing? */
				bsect = LD_DWORD(&buf[8]);	/* Partition offset in LBA */
				fmt = check_fs(buf, bsect, fs);	/* Check the partition */
			}
		}
	}
...
)
{
	...
	*br = 0;
	...
	while (btr)	{									/* Repeat until all data transferred */
		...
		dr = disk_readp(!buff ? 0 : rbuff, fs->dsect, (UINT)fs->fptr % 512, rcnt, fs);
...
)
{
	...
	if (!fs) {				/* Check file system */
		...
		dj->fn = sp;
		res = follow_path(dj, dir, path, fs);		/* Follow the path to the directory */
	...
	if (!fs) {				/* Check file system */
		...
		if (!fno) {
			...
			if (res == FR_OK) {				/* A valid entry is found */
				...
				res = dir_next(dj, fs);			/* Increment read index for next */
				if (res == FR_NO_FILE) res = FR_OK;
			}
		}
	}
