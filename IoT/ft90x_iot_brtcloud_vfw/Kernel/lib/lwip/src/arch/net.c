...
#define NET_ETHERNET_INPUT_TASK_PRIO 		(configMAX_PRIORITIES-3)
...
/** @brief Sets the MAC address of the interface in EEPROM.
 *  @returns Status of the operation
 */
int net_set_mac_eeprom(const uint8_t *mac)
{
	if (!mac) {
		return ERR_ARG;
	}
	...
	int8_t i2c_status = ee_write(NET_EEPROM_OFFSET_MACADDRESS, (uint8_t *)mac, 6);
	if (i2c_status != 0)
	{
		NET_DEBUG_PRINTF("MAC address write failed\r\n");
		return ERR_IF;
	}
