...
static inline err_t mqtt_publish_async( mqtt_client_t *client,
    const char* topic, const char* msg, int msg_len, u8_t qos );
    ...
    mqtt_client_t *client, const char* topic, iot_subscribe_cb subscribe_cb, u8_t qos )
{
    ...
    if (subscribe_cb) {
        ...
        mqtt_unsubscribe( client, topic, NULL, NULL );
    }
...
static inline err_t mqtt_publish_async(
    mqtt_client_t *client, const char* topic, const char* msg, int msg_len, u8_t qos )
{
    err_t err;
    u8_t retain = 0;
...
#else
extern iot_certificates* g_tls_certificates;
extern iot_credentials* g_mqtt_credentials;
void* iot_connect_ex(void)
{
    struct mqtt_connect_client_info_t mqtt_info = {0};
    iot_context* handle = g_handle;
    err_t err = ERR_OK;
    ...
    if (!g_tls_certificates->cert) {
        mqtt_info.tls_config = altcp_tls_create_config_client(
            g_tls_certificates->ca, g_tls_certificates->ca_len );
    }
    else {
        mqtt_info.tls_config = altcp_tls_create_config_client_2wayauth(
            g_tls_certificates->ca, g_tls_certificates->ca_len,
            g_tls_certificates->pkey, g_tls_certificates->pkey_len, NULL, 0,
            g_tls_certificates->cert, g_tls_certificates->cert_len );
    }
    ...
    mqtt_info.client_id = g_mqtt_credentials->client_id;
    mqtt_info.client_user = g_mqtt_credentials->client_user;
    mqtt_info.client_pass = g_mqtt_credentials->client_pass;
    ...
    err = mqtt_connect_async(
        &handle->mqtt,
        g_mqtt_credentials->server_host,
        g_mqtt_credentials->server_port,
        &mqtt_info );
    if ( err != ERR_OK ) {
        DEBUG_PRINTF( "mqtt_connect_async failed! %d\r\n", err );
        altcp_tls_free_config( mqtt_info.tls_config );
    ...
    DEBUG_PRINTF( "mqtt_connect_async done! %d\r\n", err );
    //
    // Wait until connection is established
    //
    do
 ...
 *  @returns Returns 0 if success, negative value err_t otherwise
 */
int iot_subscribe( void* handle, const char* topic, iot_subscribe_cb subscribe_cb, int qos )
{
    err_t err = ERR_OK;
    iot_context *_handle = ( iot_context * )handle;
    ...
    //
    err = mqtt_subscribe_async( &_handle->mqtt, topic, subscribe_cb, (u8_t)qos );
#endif // USE_MQTT_SUBSCRIBE
 ...
 *  @returns Returns None
 */
void iot_unsubscribe( void* handle, const char* topic )
{
    iot_context *_handle = ( iot_context * )handle;
    ...
    //
    mqtt_subscribe_async( &_handle->mqtt, topic, NULL, 0 );
#endif // USE_MQTT_SUBSCRIBE
}
 ...
 *  @returns Returns 0 if success, negative value err_t otherwise
 */
int iot_publish( void* handle, const char* topic, const char* payload, int payload_len, int qos )
{
    err_t err = ERR_OK;
    iot_context *_handle = ( iot_context * )handle;
    ...
    int retries = 0;
    do {
        err = mqtt_publish_async( &_handle->mqtt, topic, payload, payload_len, (u8_t)qos );
        if ( err == ERR_MEM ) {
            retries++;
            vTaskDelay( pdMS_TO_TICKS(1000) );
        }
